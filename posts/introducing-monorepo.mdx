...
이런 중복된 코드들로 인해 여러 문제점이 발생하고 있었습니다. 하나의 변경사항을 반영하기 위해 두 군데에서 코드를 모두 수정해야 했고 코드 변경 시 실수로 한 쪽만 수정하고 다른 쪽은 수정하지 않는 경우도 발생했습니다. 또한 공유되는 코드들이 의존하고 있는 패키지 버전이 서로 다를 경우, 각 프로젝트에서 사용하는 패키지 버전도 개발자가 직접 관리해야 했습니다. 이러한 문제들을 해결하기 위해 모노레포를 도입하기로 결정했습니다.
