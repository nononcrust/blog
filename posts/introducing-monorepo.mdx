...
제가 진행 중인 프로젝트는 교사와 학생이 사용하는 웹 기반 학습 관리 애플리케이션입니다. 프로젝트 특성 상 교사용 웹과 학생용 웹은 각각 다른 React 프로젝트로 구성되어 있는데요, 서로 다른 프로젝트이지만 실제로는 꽤 많은 코드가 각 프로젝트에 동일하게 작성되고 있었습니다. 직접 확인해본 결과 아래와 같은 유형의 코드들이 중복되어 나타나고 있었습니다.
